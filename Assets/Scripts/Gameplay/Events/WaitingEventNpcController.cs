...
{
    /// <summary>
    /// 等待事件 · 分支对白 NPC：流程与 <see cref="SickClassmateNpcController"/> 相同，默认台词与存档键独立。
    /// 与同物体其它 <see cref="IWorldInteractable"/> 二选一，勿同时挂两个。
    /// 未挂 Collider2D 时会自动添加 <see cref="CircleCollider2D"/>（Trigger）。
    /// </summary>
    [AddComponentMenu("Gameplay/Events/Waiting Event NPC Controller")]
    public class WaitingEventNpcController : MonoBehaviour, IWorldInteractable, IInteractionRewindTarget
    {
        [Header("对话 UI")]
        ...
        [Header("存档")]
        [Tooltip("完成 E 线后的进度键。勿与其它事件重复。留空则回退为 WaitingEvent_EventComplete。")]
        public string progressPlayerPrefsKey = "WaitingEvent_EventComplete";
        ...
        public string waitingEventRewardStorageKey = "WaitingEvent_RewardCardStored";

        [Tooltip("成就卡图案；空则不弹出奖励层")]
        public Sprite waitingEventRewardCardSprite;

        public string waitingEventRewardCardTitle = "耐心守候";
        ...
        void EnsureCollider2DTriggerEditor()
        {
            ...
            {
                Undo.RecordObject(col, "WaitingEvent Collider isTrigger");
                col.isTrigger = true;
            }
        }
#endif
        ...
        void FinishWaitingEventBranchRewardAndCalm()
        {
            ...
            if (rewardCardOfferService != null && waitingEventRewardCardSprite != null &&
                !string.IsNullOrEmpty(waitingEventRewardStorageKey))
            {
                ...
                rewardCardOfferService.Offer(
                    waitingEventRewardCardSprite,
                    waitingEventRewardCardTitle,
                    waitingEventRewardStorageKey,
        ...
        void UpdateNpcSaysQWaitSpace()
        {
            if (Input.GetKeyDown(KeyCode.Space))
            {
                ...
                FinishWaitingEventBranchRewardAndCalm();
            }
        }
        ...
        public void ResetToPreInteractState()
        {
            ...
            if (!skipPersistentProgress)
            {
                ...
                if (!string.IsNullOrEmpty(waitingEventRewardStorageKey))
                    PlayerPrefs.DeleteKey(waitingEventRewardStorageKey);
                PlayerPrefs.Save();
            }
