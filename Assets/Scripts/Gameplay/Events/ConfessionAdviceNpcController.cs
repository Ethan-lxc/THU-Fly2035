...
{
    /// <summary>
    /// 表白建议事件 · 分支对白 NPC：流程与 <see cref="SickClassmateNpcController"/> 相同，默认台词与存档键独立。
    ...
    public class ConfessionAdviceNpcController : MonoBehaviour, IWorldInteractable, IInteractionRewindTarget
    {
        ...
        [Header("存档")]
        [Tooltip("完成 E 线后的进度键。勿与其它事件重复。留空则回退为 ConfessionAdvice_EventComplete。")]
        public string progressPlayerPrefsKey = "ConfessionAdvice_EventComplete";
        ...
        public string confessionAdviceRewardStorageKey = "ConfessionAdvice_RewardCardStored";

        [Tooltip("成就卡图案；空则不弹出奖励层")]
        public Sprite confessionAdviceRewardCardSprite;

        public string confessionAdviceRewardCardTitle = "真心建议";
        ...
        string EffectiveProgressPrefsKey =>
            string.IsNullOrWhiteSpace(progressPlayerPrefsKey)
                ? "ConfessionAdvice_EventComplete"
        ...
        void EnsureCollider2DTriggerEditor()
        {
            ...
            {
                Undo.RecordObject(col, "ConfessionAdvice Collider isTrigger");
                col.isTrigger = true;
            }
        }
#endif
        ...
        void FinishConfessionAdviceBranchRewardAndCalm()
        {
            ...
            if (rewardCardOfferService != null && confessionAdviceRewardCardSprite != null &&
                !string.IsNullOrEmpty(confessionAdviceRewardStorageKey))
            {
                ...
                rewardCardOfferService.Offer(
                    confessionAdviceRewardCardSprite,
                    confessionAdviceRewardCardTitle,
                    confessionAdviceRewardStorageKey,
        ...
        void UpdateNpcSaysQWaitSpace()
        {
            if (Input.GetKeyDown(KeyCode.Space))
            {
                ...
                FinishConfessionAdviceBranchRewardAndCalm();
            }
        }
        ...
        public void ResetToPreInteractState()
        {
            ...
            if (!skipPersistentProgress)
            {
                ...
                if (!string.IsNullOrEmpty(confessionAdviceRewardStorageKey))
                    PlayerPrefs.DeleteKey(confessionAdviceRewardStorageKey);
                PlayerPrefs.Save();
            }
