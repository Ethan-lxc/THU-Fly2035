...
{
    /// <summary>
    /// 送伞事件 · 分支对白 NPC：流程与 <see cref="SickClassmateNpcController"/> 相同，默认台词与存档键独立。
    /// 与同物体其它 <see cref="IWorldInteractable"/> 二选一，勿同时挂两个。
    /// 未挂 Collider2D 时会自动添加 <see cref="CircleCollider2D"/>（Trigger）。
    /// </summary>
    [AddComponentMenu("Gameplay/Events/Umbrella Delivery NPC Controller")]
    public class UmbrellaDeliveryNpcController : MonoBehaviour, IWorldInteractable, IInteractionRewindTarget
    {
        [Header("对话 UI")]
        ...
        [Header("存档")]
        [Tooltip("完成 E 线后的进度键。勿与其它事件重复。留空则回退为 UmbrellaDelivery_EventComplete。")]
        public string progressPlayerPrefsKey = "UmbrellaDelivery_EventComplete";
        ...
        public string umbrellaDeliveryRewardStorageKey = "UmbrellaDelivery_RewardCardStored";

        [Tooltip("成就卡图案；空则不弹出奖励层")]
        public Sprite umbrellaDeliveryRewardCardSprite;

        public string umbrellaDeliveryRewardCardTitle = "雨中同行";
        ...
        string EffectiveProgressPrefsKey =>
            string.IsNullOrWhiteSpace(progressPlayerPrefsKey)
                ? "UmbrellaDelivery_EventComplete"
        ...
        void EnsureCollider2DTriggerEditor()
        {
            ...
            {
                Undo.RecordObject(col, "UmbrellaDelivery Collider isTrigger");
                col.isTrigger = true;
            }
        }
#endif
        ...
        void FinishUmbrellaDeliveryBranchRewardAndCalm()
        {
            ...
            if (rewardCardOfferService != null && umbrellaDeliveryRewardCardSprite != null &&
                !string.IsNullOrEmpty(umbrellaDeliveryRewardStorageKey))
            {
                ...
                rewardCardOfferService.Offer(
                    umbrellaDeliveryRewardCardSprite,
                    umbrellaDeliveryRewardCardTitle,
                    umbrellaDeliveryRewardStorageKey,
        ...
        void UpdateNpcSaysQWaitSpace()
        {
            if (Input.GetKeyDown(KeyCode.Space))
            {
                ...
                FinishUmbrellaDeliveryBranchRewardAndCalm();
            }
        }
        ...
        public void ResetToPreInteractState()
        {
            ...
            if (!skipPersistentProgress)
            {
                ...
                if (!string.IsNullOrEmpty(umbrellaDeliveryRewardStorageKey))
                    PlayerPrefs.DeleteKey(umbrellaDeliveryRewardStorageKey);
                PlayerPrefs.Save();
            }
