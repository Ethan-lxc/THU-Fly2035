...
{
    /// <summary>
    /// 失眠建议事件 · 分支对白 NPC：流程与 <see cref="SickClassmateNpcController"/> 相同，默认台词与存档键独立。
    /// 与同物体其它 <see cref="IWorldInteractable"/> 二选一，勿同时挂两个。
    /// 未挂 Collider2D 时会自动添加 <see cref="CircleCollider2D"/>（Trigger）。
    /// </summary>
    [AddComponentMenu("Gameplay/Events/Insomnia Advice NPC Controller")]
    public class InsomniaAdviceNpcController : MonoBehaviour, IWorldInteractable, IInteractionRewindTarget
    {
        [Header("对话 UI")]
        ...
        [Header("存档")]
        [Tooltip("完成 E 线后的进度键。勿与其它事件重复。留空则回退为 InsomniaAdvice_EventComplete。")]
        public string progressPlayerPrefsKey = "InsomniaAdvice_EventComplete";
        ...
        public string insomniaAdviceRewardStorageKey = "InsomniaAdvice_RewardCardStored";

        [Tooltip("成就卡图案；空则不弹出奖励层")]
        public Sprite insomniaAdviceRewardCardSprite;

        public string insomniaAdviceRewardCardTitle = "良好睡眠";
        ...
        void EnsureCollider2DTriggerEditor()
        {
            ...
            {
                Undo.RecordObject(col, "InsomniaAdvice Collider isTrigger");
                col.isTrigger = true;
            }
        }
#endif
        ...
        void FinishInsomniaBranchRewardAndCalm()
        {
            ...
            if (rewardCardOfferService != null && insomniaAdviceRewardCardSprite != null &&
                !string.IsNullOrEmpty(insomniaAdviceRewardStorageKey))
            {
                ...
                rewardCardOfferService.Offer(
                    insomniaAdviceRewardCardSprite,
                    insomniaAdviceRewardCardTitle,
                    insomniaAdviceRewardStorageKey,
        ...
        void UpdateNpcSaysQWaitSpace()
        {
            if (Input.GetKeyDown(KeyCode.Space))
            {
                ...
                FinishInsomniaBranchRewardAndCalm();
            }
        }
        ...
        public void ResetToPreInteractState()
        {
            ...
            if (!skipPersistentProgress)
            {
                ...
                if (!string.IsNullOrEmpty(insomniaAdviceRewardStorageKey))
                    PlayerPrefs.DeleteKey(insomniaAdviceRewardStorageKey);
                PlayerPrefs.Save();
            }
